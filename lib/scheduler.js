/**
 * 주기 작업 스케줄러: 서버 시작 시 실행되는 모든 setInterval 작업 관리
 *
 * 작성자: 최준호
 * 작성일: 2026-03-12
 */
...
import { getLastConsolidation, recordConsolidationResult } from "./consolidation-observability.js";

export { getLastConsolidation };
...
export function startSchedulers({ globalEmbeddingWorkerRef }) {
  ...
  setInterval(async () => {
    ...
    try {
      ...
      const recorded = recordConsolidationResult(result);
  ...
  /** Redis 인덱스 워밍업: cold start L1 miss 감소 (비치명적) */
  getFragmentIndex()
  ...
  /** NLI 모델 사전 로드 (cold start 방지, 비치명적) */
