...
export class MemoryConsolidator {
  ...
  async consolidate() {
    ...
    const results = {
      ...
      stale_fragments          : [],
      reflectionsPurged        : 0,
      noisyFragmentsPurged     : 0,
      noisyMorphemesPurged     : 0,
    ...
    try {
      ...
      /** 1. TTL 계층 전환 (전환 수 추적) */
      const s1 = await timedStage("ttl_transition", () => this._transitionWithCount());
      stages.push(s1);
      results.ttlTransitions = s1.affected;

      /** 2. 중요도 감쇠 */
      const s2 = await timedStage("importance_decay", async () => {
      ...
      /** 3. 만료 파편 삭제 */
      const s3 = await timedStage("expired_delete", () => this.store.deleteExpired());
      stages.push(s3);
      results.expiredDeleted = s3.affected;

      /** 3.5. GC 후보 분포 미리보기 */
      const s35 = await timedStage("gc_preview", async () => {
      ...
      const s37 = await timedStage("split_long_fragments", () => this._splitLongFragments());
      stages.push(s37);
      results.fragmentsSplit = s37.affected;

      /** 4. 중복 파편 병합 */
      const s4 = await timedStage("merge_duplicates", () => this._mergeDuplicates());
      stages.push(s4);
      results.duplicatesMerged = s4.affected;

      /** 4.5. 시맨틱 중복 제거 (cos >= 0.92, topic별 KNN) */
      const s45 = await timedStage("semantic_dedup", () => this._semanticDedup());
      stages.push(s45);
      results.semanticDedupMerged = s45.affected;

      /** 4.7. 오래된 파편 압축 (cos >= 0.80, importance < 0.5) */
      const s47 = await timedStage("compress_old_fragments", () => this._compressOldFragments());
      stages.push(s47);
      results.compressedFragments = s47.affected;

      /** 5. 누락 임베딩 보충 */
      const s5 = await timedStage("embeddings_backfill", async () => {
      ...
      const s55 = await timedStage("retro_link", async () => {
      ...
      /** 6. utility_score 갱신 */
      const s6 = await timedStage("utility_score_update", () => this._updateUtilityScores());
      stages.push(s6);
      results.utilityUpdated = s6.affected;

      /** 6.2. 고-EMA 저-importance 파편 재평가 큐 등록 */
      const s62 = await timedStage("requeue_high_ema", () => this._requeueHighEmaLowQuality());
      stages.push(s62);
      if (s62.affected > 0) {
      ...
      const s77 = await timedStage("detect_supersessions", () => this._detectSupersessions());
      stages.push(s77);
      results.supersessionsDetected = s77.affected;

      /** 7.5. 보류 중인 모순 후처리 */
      const s75 = await timedStage("process_pending_contradictions", () => this._processPendingContradictions());
      stages.push(s75);
      results.pendingContradictions = s75.affected;

      /** 8. 피드백 리포트 생성 */
      const s8 = await timedStage("feedback_report", () => this._generateFeedbackReport());
      stages.push(s8);
      results.feedbackReportGenerated = s8.status === "ok" && s8.affected > 0;

      /** 8.5. 피드백 적응형 importance 보정 */
      const s85 = await timedStage("feedback_calibration", () => this._calibrateByFeedback());
      stages.push(s85);
      if (s85.affected > 0) {
      ...
      /** 9. Redis 인덱스 정리 */
      const s9 = await timedStage("prune_keyword_indexes", async () => {
      ...
      /** 10. stale 파편 목록 수집 */
      let staleList = [];
      const s10 = await timedStage("collect_stale_fragments", async () => {
      ...
      const s11 = await timedStage("purge_stale_reflections", () => this._purgeStaleReflections());
      stages.push(s11);
      results.reflectionsPurged = s11.affected;

      /** 11.5. synthetic/test/diff 계열 파편 정리 */
      results.noisyFragmentsPurged = await this._purgeNoisyFragments();

      /** 11.7. 형태소 사전 노이즈 정리 */
      results.noisyMorphemesPurged = await this._purgeNoisyMorphemes();

      /** 11.9. 정리 후 Redis 인덱스 재정리 */
      await this.index.pruneKeywordIndexes();

      /** 12. search_events 30일 초과 레코드 정리 */
      const s12 = await timedStage("gc_search_events", () => this._gcSearchEvents());
      stages.push(s12);
  ...
  async _purgeNoisyFragments() {
    return this.consolidatorGC.purgeNoisyFragments();
  }

  async _purgeNoisyMorphemes() {
    return this.consolidatorGC.purgeNoisyMorphemes();
  }
