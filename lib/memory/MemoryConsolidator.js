...
export class MemoryConsolidator {
  ...
  async _detectContradictions() {
    ...
    } catch (err) { logWarn(`[MemoryConsolidator] Redis lastCheckAt read failed: ${err.message}`); }
    ...
    for (const newFrag of newFrags.rows) {
      /** Stage 1: pgvector 코사인 유사도 > 0.85 + 기존 contradicts 링크 제외 */
      ...
      for (const candidate of candidates.rows) {
        ...
        try {
          ...
          logWarn(`[MemoryConsolidator] Gemini contradiction check failed: ${err.message}`);
        }
      }
    }
    ...
    if (nliResolved > 0 || nliSkipped > 0) {
      logInfo(`[MemoryConsolidator] NLI stats: ${nliResolved} resolved, ${nliSkipped} skipped (saved ${nliResolved + nliSkipped} Gemini calls)`);
    }
  ...
  async _resolveContradiction(newFrag, candidate, reasoning) {
    ...
    /** 모순 해결 결과 audit trail: decision 파편으로 기록 */
    try {
   ...
   *
   * similarity > 0.85는 GraphLinker가 이미 처리하므로 여기서는 중간 구간만 다룬다.
   * valid_to가 이미 설정된 파편은 건너뛴다.
   *
   * @returns {number} 감지된 supersession 수
   */
  async _detectSupersessions() {
    ...
    for (const pair of candidates.rows) {
      try {
        const verdict = await this._askGeminiSupersession(
        ...
        if (verdict.supersedes) {
          ...
          logInfo(`[MemoryConsolidator] Supersession: ${older.id} -> ${newer.id}: ${verdict.reasoning}`);
          detected++;
        }
      } catch (err) {
        logWarn(`[MemoryConsolidator] Supersession check failed: ${err.message}`);
      }
    }
...
{"supersedes": true 또는 false, "reasoning": "판단 근거 1문장"}`;
    ...
    try {
      ...
      logWarn(`[MemoryConsolidator] Gemini supersession parse failed: ${err.message}`);
...
{"contradicts": true 또는 false, "reasoning": "판단 근거 1문장"}`;
    ...
    try {
      ...
      logWarn(`[MemoryConsolidator] Gemini CLI parse failed: ${err.message}`);
  ...
  async _flagPotentialContradiction(redisClient, key, fragA, fragB) {
    try {
      if (redisClient && redisClient.status === "ready") {
        ...
        logDebug(`[MemoryConsolidator] Flagged potential contradiction: ${fragA.id} <-> ${fragB.id}`);
      }
    } catch (err) {
      logWarn(`[MemoryConsolidator] Failed to flag contradiction: ${err.message}`);
    }
  }
  ...
  async _processPendingContradictions() {
    ...
    for (let i = 0; i < batchSize; i++) {
      ...
      try {
        ...
        logWarn(`[MemoryConsolidator] Pending contradiction processing failed: ${err.message}`);
        /** 실패 시 다시 큐 뒤로 밀어넣기 */
    ...
    if (processed > 0) {
      logInfo(`[MemoryConsolidator] Processed ${processed} pending contradictions`);
    }
    return processed;
  }
  ...
  async _updateContradictionTimestamp(redisClient, key, timestamp) {
    ...
    } catch (err) { logWarn(`[MemoryConsolidator] Contradiction timestamp update failed: ${err.message}`); }
  }
