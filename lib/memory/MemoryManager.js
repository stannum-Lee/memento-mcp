...
export class MemoryManager {
  ...
  async link(params) {
    ...
    /** API 키 소유권 검사: 마스터가 아니면 두 파편 모두 소유해야 함 */
    if (keyId && (fromFrag.key_id !== keyId || toFrag.key_id !== keyId)) {
  ...
  async context(params) {
    ...
    try {
      ...
      if (pool) {
        ...
        let   anchorKeyFilter = "";
        if (keyId != null) {
          anchorParams.push(keyId);
          anchorKeyFilter = `AND key_id = $${anchorParams.length}`;
        }
  ...
  async fragmentHistory(params) {
    ...
    /** API 키 소유권 검사 */
    if (keyId && result.current.key_id !== keyId) {
  ...
  async graphExplore(params) {
    ...
    if (keyId && startFrag.key_id !== keyId) {
      return { error: "이 파편에 대한 조회 권한이 없습니다." };
    }

    const nodes = await this.store.getRCAChain(params.startId, agentId, keyId);
