...
import { isNoiseLikeFragment } from "./NoiseFilters.js";
...
export class MemoryManager {
  ...
  async batchRemember(params) {
    ...
    try {
        ...
        try {
          ...
          const insertSql = `INSERT INTO ${SCHEMA}.fragments
                    (id, content, topic, keywords, type, importance, content_hash,
                     ...
                     workspace, context_summary, session_id, embedding)
                 ...
                 ON CONFLICT (content_hash) DO UPDATE SET
                    ...
                    context_summary = COALESCE(${SCHEMA}.fragments.context_summary, EXCLUDED.context_summary),
                    session_id = COALESCE(${SCHEMA}.fragments.session_id, EXCLUDED.session_id),
                    accessed_at = NOW()
                 RETURNING id`;
      ...
      for (const { fragment } of validFragments) {
        ...
        this.index.index({ ...fragment, id: results[idx].id }, fragment.session_id || null, keyId).catch(() => {});
  ...
  async _supersede(oldId, newId, agentId = "default", keyId = null) {
    const existing = await this.store.getById(oldId, agentId);
    const result = await this.conflictResolver.supersede(oldId, newId, agentId, keyId);
  ...
  async reflect(params) {
    ...
    if (params.summary) {
      ...
      for (const item of summaryItems) {
        const saved = await this._rememberReflectFragment({
    ...
    if (params.errors_resolved && params.errors_resolved.length > 0) {
      for (const err of params.errors_resolved) {
        ...
        const candidateContent = `[해결됨] ${err.trim()}`;
        if (isNoiseLikeFragment({ topic: "session_reflect", content: candidateContent, source: sessionSrc })) continue;
    ...
    if (params.new_procedures && params.new_procedures.length > 0) {
      for (const proc of params.new_procedures) {
        ...
        if (isNoiseLikeFragment({ topic: "session_reflect", content: proc.trim(), source: sessionSrc })) continue;
        const f = this.factory.create({
          content    : proc.trim(),
          topic      : "session_reflect",
          type       : "procedure",
    ...
    if (params.open_questions && params.open_questions.length > 0) {
      for (const q of params.open_questions) {
        ...
        const candidateContent = `[미해결] ${q.trim()}`;
        if (isNoiseLikeFragment({ topic: "session_reflect", content: candidateContent, source: sessionSrc })) continue;
        const f = this.factory.create({
          content    : `[미해결] ${q.trim()}`,
          topic      : "session_reflect",
   ...
   */
  async _rememberReflectFragment(params) {
    const content = String(params.content || "").trim();
    if (!content) return null;

    if (isNoiseLikeFragment({
      topic: params.topic,
      content,
    ...
    fragment.agent_id = params.agentId;
    fragment.key_id = params.keyId ?? null;

    const id = await this.store.insert(fragment);
    await this.index.index({ ...fragment, id }, params.sessionId, fragment.key_id ?? null);
    return { id, content: fragment.content, type: fragment.type, keywords: fragment.keywords };
  }
  ...
  async graphExplore(params) {
    ...
    const edges = nodes
      ...
      .map(n => ({
        from         : n.edge_from || params.startId,
        to           : n.edge_to || n.id,
