...
export class FragmentSearch {
  ...
  async search(query) {
    ...
    const searchPath   = [];
    this._anchorTime   = query.anchorTime || Date.now();
    let   combined    = [];
  ...
  _deduplicate(fragments, fragmentCount = 0) {
    ...
    return allFragments.sort((a, b) =>
      this._computeRankScore(b, MEMORY_CONFIG, this._anchorTime)
      - this._computeRankScore(a, MEMORY_CONFIG, this._anchorTime)
    );
  }
