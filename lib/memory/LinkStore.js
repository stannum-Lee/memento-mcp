      ...
      result = await queryWithAgentVector(agentId,
        `SELECT DISTINCT ON (id)
         ...
         FROM (
           ...
           JOIN ${SCHEMA}.fragments f ON l.to_id = f.id
           WHERE l.from_id = ANY($1)
             AND l.relation_type IN ('caused_by', 'resolved_by', 'related', 'part_of', 'contradicts', 'co_retrieved')
             ${keyFilter}
             ${temporalFilter}
           UNION ALL
           SELECT f.id, f.content, f.topic, f.keywords, f.type,
    ...
    const result = await queryWithAgentVector(agentId,
      `WITH rca AS (
         SELECT f.id, f.content, f.type, f.importance, f.topic,
                ...
                NULL::text AS edge_from, NULL::text AS edge_to
         ...
         SELECT f2.id, f2.content, f2.type, f2.importance, f2.topic,
                ...
                l.from_id AS edge_from, l.to_id AS edge_to
         FROM ${SCHEMA}.fragment_links l
         JOIN ${SCHEMA}.fragments f2 ON f2.id = CASE
           WHEN l.from_id = $1 THEN l.to_id
           ELSE l.from_id
         END
         WHERE (
             (l.from_id = $1 AND l.relation_type = 'caused_by')
          OR (l.relation_type = 'resolved_by' AND (l.from_id = $1 OR l.to_id = $1))
         )
           ${keyFilter}
       )
