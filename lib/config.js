...
const ENV_FILE = process.env.MEMENTO_ENV_FILE || path.join(REPO_ROOT, ".env");
...
const defaultLogDir = process.platform === "win32" ? "logs" : "/var/log/mcp";
const rawLogDir = process.env.LOG_DIR || defaultLogDir;
const dbNameFromUrl = databaseUrl?.pathname
  ? decodeURIComponent(databaseUrl.pathname.replace(/^\/+/, ""))
  : "";
const dbUserFromUrl = databaseUrl?.username
  ? decodeURIComponent(databaseUrl.username)
  : "";
const dbPasswordFromUrl = databaseUrl?.password
...
export const LOG_DIR            = path.isAbsolute(rawLogDir) ? rawLogDir : path.resolve(REPO_ROOT, rawLogDir);
...
const _PROVIDER_DEFAULTS = {
  ...
  transformers: { model: "Xenova/bge-m3",     dims: 1024,  baseUrl: "",                                                        supportsDimensionsParam: false },
...
export const DB_HOST            = process.env.POSTGRES_HOST || process.env.DB_HOST || databaseUrl?.hostname || "";
export const DB_PORT            = Number(process.env.POSTGRES_PORT || process.env.DB_PORT || databaseUrl?.port || 5432);
export const DB_NAME            = process.env.POSTGRES_DB || process.env.DB_NAME || dbNameFromUrl;
export const DB_USER            = process.env.POSTGRES_USER || process.env.DB_USER || dbUserFromUrl;
export const DB_PASSWORD        = process.env.POSTGRES_PASSWORD || process.env.DB_PASSWORD || dbPasswordFromUrl;
