...
export async function validateApiKeyFromDB(rawKey) {
  ...
  const { rows }    = await pool.query(`
    SELECT
      ...
      k.status,
      k.daily_limit,
      COALESCE(m.group_id, k.id) AS effective_key_id,
      COALESCE(u.call_count, 0)  AS usage_today
    FROM  agent_memory.api_keys k
    LEFT JOIN LATERAL (
      SELECT group_id
      FROM   agent_memory.api_key_group_members
      WHERE  key_id = k.id
  ...
  return { valid: true, keyId: key.effective_key_id, permissions: key.permissions };
}
