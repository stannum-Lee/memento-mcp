...
/**
 * GET /v1/internal/model/nothing/assets/:file (Admin 정적 파일: CSS, JS)
 */
export function handleAdminStatic(req, res) {
  const url      = new URL(req.url || "/", "http://localhost");
  const filename = path.basename(url.pathname);
  const allowed  = { "admin.css": "text/css", "admin.js": "text/javascript" };
  const mime     = allowed[filename];
  if (!mime) {
    res.statusCode = 404;
    res.end("Not found");
    return;
  }
  const filePath = path.join(__dirname, "..", "..", "assets", "admin", filename);
  fs.readFile(filePath, (err, data) => {
    if (err) { res.statusCode = 404; res.end("Asset not found"); return; }
    res.statusCode = 200;
