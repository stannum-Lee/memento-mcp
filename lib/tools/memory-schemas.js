    ...
    properties: {
      fragments: {
        ...
        items      : {
          type      : "object",
          properties: {
            content   : { type: "string", description: "???????? (1~3???, 300????? ???)" },
            ...
            /*
            importance: { type: "number", minimum: 0, maximum: 1, description: "?????0~1" },
            keywords  : { type: "array", items: { type: "string" }, description: "????? ?????" },
            source    : { type: "string", description: "?? (?: session:<id>, file:<path>)" },
            contextSummary: {
              type: "string",
              description: "????/?? ??? ?? ??. recall(includeContext)?? ?? ???."
            },
            sessionId: {
              type: "string",
              description: "?? ?? episode/working-memory ??? ??? ?? ID."
            }
            ...
            */
            importance: { type: "number", minimum: 0, maximum: 1, description: "중요도 0~1" },
            keywords  : { type: "array", items: { type: "string" }, description: "검색용 키워드" },
            source    : { type: "string", description: "출처 (예: session:<id>, file:<path>)" },
            contextSummary: {
              type: "string",
              description: "조각의 배경 문맥 요약. recall(includeContext)에서 후속 이해를 돕는다."
            },
            sessionId: {
              type: "string",
              description: "관련 세션 ID. episode/working-memory 연결에 사용된다."
            },
            workspace : {
              type: "string",
...
export const reflectDefinition = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      narrative_summary: {
        type       : "string",
        description: "Session narrative summary stored as an episode fragment and reused by includeContext/sessionId flows."
      },
      task_effectiveness: {
        type       : "object",
