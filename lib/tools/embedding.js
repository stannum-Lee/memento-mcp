...
let transformersExtractor = null;
let transformersLoading = null;
...
async function getTransformersExtractor() {
  ...
  transformersLoading = (async () => {
    ...
    const cacheDir = process.env.MEMENTO_TRANSFORMERS_CACHE
    ...
    transformersExtractor = await pipeline("feature-extraction", EMBEDDING_MODEL, {
      dtype: "q8"
    });
  ...
  try {
    return await transformersLoading;
  } finally {
...
function extractVector(output) {
  ...
  if (typeof output?.tolist === "function") {
    return output.tolist().flat(Infinity);
  }
  throw new Error("Unsupported transformers embedding output shape");
}
...
export async function generateEmbedding(text) {
  if (EMBEDDING_PROVIDER === "transformers") {
    ...
    return normalizeL2(extractVector(output));
  }
...
export async function generateBatchEmbeddings(texts, batchSize = 100) {
  if (EMBEDDING_PROVIDER === "transformers") {
    ...
    for (const text of texts) {
      allVectors.push(await generateEmbedding(text));
    }
    return allVectors;
  }
