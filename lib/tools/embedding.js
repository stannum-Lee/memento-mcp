...
/** In-process transformers 모델 (lazy 초기화) */
let _pipeline  = null;
let _loading   = null;
let _loadFailed = false;

/**
 * @huggingface/transformers 기반 in-process 임베딩 파이프라인 (싱글턴)
 * NLIClassifier.js와 동일한 패턴으로 ONNX 모델을 프로세스 내에서 실행한다.
 */
async function getTransformersPipeline() {
  if (_pipeline) return _pipeline;
  if (_loadFailed) throw new Error("Transformers 임베딩 모델 로드에 실패했습니다.");
  ...
  _loading = (async () => {
    try {
      ...
      console.log(`[Embedding] Loading transformers model: ${EMBEDDING_MODEL} ...`);
      const t0 = Date.now();
      ...
      console.log(`[Embedding] Transformers model ready in ${sec}s (dims: ${EMBEDDING_DIMENSIONS})`);

      return _pipeline;
    } catch (err) {
      console.error(`[Embedding] Transformers model load failed: ${err.message}`);
      _loadFailed = true;
      throw err;
    } finally {
      _loading = null;
...
/**
 * In-process transformers로 임베딩 생성
 * @param {string} text
 * @returns {Promise<number[]>}
 */
async function generateEmbeddingTransformers(text) {
  const pipe = await getTransformersPipeline();
  const output = await pipe(text, { pooling: "cls", normalize: true });

  // Tensor → number[]로 변환
  let vec = Array.from(output.data);

  // 차원 조정 (모델 출력 > 설정 차원이면 truncate)
  if (vec.length > EMBEDDING_DIMENSIONS) {
    vec = vec.slice(0, EMBEDDING_DIMENSIONS);
  }
...
export async function generateBatchEmbeddings(texts, batchSize = 100) {
  if (EMBEDDING_IN_PROCESS) {
    ...
    for (const text of texts) {
      allVectors.push(await generateEmbeddingTransformers(text));
    }
    return allVectors;
  }
...
export { OPENAI_API_KEY, EMBEDDING_API_KEY, EMBEDDING_ENABLED, EMBEDDING_IN_PROCESS, EMBEDDING_MODEL, EMBEDDING_DIMENSIONS, EMBEDDING_SUPPORTS_DIMS_PARAM };
