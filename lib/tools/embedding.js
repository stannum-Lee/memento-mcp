...
async function createVoyageEmbeddings(inputs, inputType = "document") {
  if (!EMBEDDING_API_KEY) {
    throw new Error("VOYAGE_API_KEY environment variable is not set");
  }

  const response = await fetch(EMBEDDING_BASE_URL || "https://api.voyageai.com/v1/embeddings", {
    method: "POST",
    headers: {
  ...
  if (!Array.isArray(payload?.data)) {
    throw new Error("Voyage embedding response is missing data[].");
  }

  return payload.data.map((item) => normalizeL2(item.embedding));
}
...
export async function generateEmbedding(text, options = {}) {
  if (isVoyageProvider()) {
    const vectors = await createVoyageEmbeddings([text], options.inputType || "document");
    return vectors[0];
  }
 ...
 */
export async function generateBatchEmbeddings(texts, batchSize = 100, options = {}) {
  if (isVoyageProvider()) {
    const allVectors = [];

    for (let i = 0; i < texts.length; i += batchSize) {
      const batch = texts.slice(i, i + batchSize);
      const vectors = await createVoyageEmbeddings(batch, options.inputType || "document");
