...
export const reflectDefinition = {
  ...
  inputSchema: {
    ...
    properties: {
      summary: {
        type       : "string",
        description: "세션 전체 요약 텍스트 (sessionId만 있으면 세션 파편에서 자동 생성)"
      },
      sessionId: {
        type       : "string",
...
export async function tool_recall(args) {
  ...
  try {
    /**
     * asOf 파라미터 존재 시 Point-in-time 조회로 분기.
     * FragmentStore.searchAsOf()가 직접 PostgreSQL 쿼리를 수행하므로
     * 일반 recall 경로(FragmentSearch)를 우회한다.
     */
    if (args.asOf) {
      const agentId = args.agentId || "default";
      const rows    = await mgr.store.searchAsOf(args.asOf, agentId, {
        limit : args.tokenBudget ? Math.ceil(args.tokenBudget / 20) : 50,
        topic : args.topic,
        type  : args.type
      });
      SessionActivityTracker.record(sessionId, {
        tool: "recall", keywords: ["asOf", args.asOf]
      }).catch(() => {});
      return {
        success    : true,
        fragments  : rows,
        count      : rows.length,
        totalTokens: rows.reduce((acc, f) => acc + (f.estimated_tokens || 0), 0),
        searchPath : "temporal",
        asOf       : args.asOf
      };
    }
