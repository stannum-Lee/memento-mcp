...
마스터 키 입력 화면이 나타나면 `MEMENTO_ACCESS_KEY` 환경변수에 설정한 키를 입력한다. URL에 `?key=` 파라미터를 붙이면 로그인 화면을 건너뛸 수 있다.

```
https://{도메인}/v1/internal/model/nothing/?key={마스터키}
```

인증에 성공하면 세션이 브라우저 탭을 닫을 때까지 유지된다.

---

## 화면 구성

좌측 사이드바에 6개 메뉴가 있다. 각 메뉴의 역할과 읽는 법을 설명한다.
