...
| `tool_feedback` | 검색 품질 피드백. fragment_ids 전달 시 해당 파편의 활성화 점수 조정 |
...
| `EVALUATOR_MAX_QUEUE` | MemoryEvaluator 큐 크기 상한 (초과 시 드롭) | `100` |
...
- RBAC: API 키별 read/write/admin 권한 분리
- 파편 내보내기/가져오기 API (JSON Lines 스트림)
- 지식 그래프 시각화 (D3.js force-directed, Admin 탭 추가)
- DB 마이그레이션 자동 러너 (`npm run migrate`)
- 학습 루프: recall 결과 피드백이 파편 활성화 점수에 반영
- 온도 기반 컨텍스트 정렬: 최근 접근/학습 파편 우선 주입
...
# 기존 설치 업그레이드: 자동 마이그레이션 (v1.8.0+, 권장)
npm run migrate    # schema_migrations 테이블 기반 증분 적용

# 또는 수동 마이그레이션: 순서대로 실행
